...
皆是$X$內的内部保持开集族.\quad$\square$
\end{fact}

\begin{definition}
\textnormal{(Junnila)} $X$的覆盖$\mathscr{D}$是半开的,如果
$\forall x\in X, x\in(\textnormal{st}(x, \mathscr{D}))^\circ$, 即, 
$\textnormal{st}(x, \mathscr{D})\in\textnormal{top}(x)$.
\end{definition}

\begin{fact}
$X$的覆盖$\mathscr{D}$是半开的$\Leftrightarrow \forall A\subset X, \overline{A} \subset\textnormal{st}(A, \mathscr{D}).\quad\square$
\end{fact}
