...
\textnormal{([Smi80], [PS89a])}	让性质$P$代表离散$(D)$,局部有限的$(LF)$和闭的$(C)$.设
$\delta$是可数序数.空间$X$是
...
对于的$P=C$情形,还要求每个$\mathscr{E}_\beta)$是$\mathscr{U}$的1-1部分加细.
...
这是因在定义\ref{def1.2.3} 中,当$\delta=\omega$时,从条件 (i) 可推出 (ii).
...
\begin{definition}
   ...
   $\mathscr{F}=\bigcup_{\beta<\delta}\mathscr{F}_\beta$使得
\end{definition}
